...
WHERE A.DATE = '20251229'
说明：
你可以直接执行这个明细 SQL，查看每一笔账户余额的原始记录，
再自行 SUM(A.BALANCE) 验证是否与报表结果一致。
...
复制代码
SELECT
    SUM(A.TRANS_AMT) AS 交易金额合计
FROM TABLE_INCOME A
WHERE A.DATE >= '20250101'
  AND A.ITEM_NO = '001'
工具自动生成明细 SQL
sql
复制代码
...
Made with ❤️ by ice789ice ｜ 2025.12.29
